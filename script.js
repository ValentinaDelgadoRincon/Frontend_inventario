...
function displayHistorialTable(data) {
  ...
  if (data.length === 0) {
    container.innerHTML =
      '<div class="message warning">No hay movimientos en el período seleccionado</div>';
    return;
  }
  ...
  google.script.run
    .withSuccessHandler((data) => {
      ...
      if (data.errores.length === 0) {
        html +=
          '<div class="message success">Todos los datos están correctos. El sistema está íntegro.</div>';
      } else {
        html +=
...
function inicializarSistema() {
  if (
    confirm(
      "¿Está seguro de que desea inicializar el sistema? Esto creará las hojas necesarias si no existen."
    )
  ) {
    google.script.run
      .withSuccessHandler((mensaje) => {
...
function verDetalleProducto(codigo) {
  alert(
    `Funcionalidad de detalle para producto: ${codigo}\nEsta función se implementará próximamente.`
  );
}
...
function confirmarReset() {
  if (
    confirm(
      "ADVERTENCIA: Esta acción eliminará TODOS los datos del sistema.\n\n¿Está completamente seguro de que desea continuar?"
    )
  ) {
    if (
      confirm(
      ...
      alert(
        "Funcionalidad de reset no implementada por seguridad.\nContacte al administrador del sistema."
      );
    }
  }
}
